...
import MobileExperienceGate, { wasMobileGuideDismissed } from "./MobileExperienceGate";
import ExperienceCoach from "./ExperienceCoach";
...
export default function Desktop() {
  ...
  // Mobile is intentionally a companion experience. Set expectations before
  // booting while preserving a clear path to the touch-friendly launcher.
  useEffect(() => {
    if (isMobile !== true || wasMobileGuideDismissed()) return;
  ...
  // Global keys: Cmd/Ctrl+K or Cmd/Ctrl+Space Spotlight; Cmd+W close window; Cmd+M minimize;
    ...
    >
      ...
      {phase === "desktop" && (
        <>
          ...
          >
            {/* Ambient widgets share the desktop stacking context: they sit
                above the surface and remain clickable, while windows with z>=10
                naturally cover them like real desktop content. */}
      ...
      <AnimatePresence>
        {mobileGateOpen && (
          <MobileExperienceGate onContinue={() => setMobileGateOpen(false)} />
        )}
      </AnimatePresence>
