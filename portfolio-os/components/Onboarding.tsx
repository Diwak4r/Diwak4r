...
export function LoginScreen({ onDone }: { onDone: () => void }) {
  ...
  const enter = () => {
    ...
    if (desktopPointer) document.documentElement.requestFullscreen?.().catch(() => {});
    onDone();
  };
  ...
  return (
    <motion.div
      ...
      className="absolute inset-0 z-[90] flex min-h-[100dvh] flex-col items-center bg-black/30 backdrop-blur-2xl"
    >
      ...
      <div className="mt-auto flex flex-col items-center pb-[16vh]">
        ...
        <button
          onClick={enter}
          className="mt-6 flex min-h-[48px] items-center rounded-full bg-white/15 px-7 py-2 text-[13.5px] font-medium text-white backdrop-blur transition hover:bg-white/25 active:scale-[0.97]"
        >
          Enter Portfolio
        </button>
