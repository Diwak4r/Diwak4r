...
function DockAppMenu({ app, onClose }: { app: AppDef; onClose: () => void }) {
  ...
  const activate = useWindows((s) => s.activate);
...
function DockItem({ app, mouseX }: { app: AppDef; mouseX: MotionValue<number> }) {
  ...
  const windows = useWindows((s) => s.wins);
  const activate = useWindows((s) => s.activate);
  const ownWindows = Object.values(windows)
    .filter((w) => w.appId === app.id)
    .sort((a, b) => b.z - a.z);
  const topWindow = ownWindows[0];
  ...
  const launch = () => {
    ...
    // Clicking a running app behaves like real macOS: restore/focus its most
    // recent window. This fixes minimized apps feeling "lost" in the Dock.
    if (topWindow) activate(topWindow.winId);
    else openApp(app.id);
  };
