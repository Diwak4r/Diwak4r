...
interface WindowStore {
  ...
  /** Restore a minimized window and bring that exact instance to the front. */
  activate: (winId: string) => void;
}
...
export const useWindows = create<WindowStore>((set) => ({
  ...
  activate: (winId) =>
    set((s) => {
      const win = s.wins[winId];
      if (!win) return s;
      const z = s.topZ + 1;
      return {
        topZ: z,
        wins: { ...s.wins, [winId]: { ...win, minimized: false, z } },
      };
    }),
}));
